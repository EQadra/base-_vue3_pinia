...
>
  <Menu size="24" />
        ...
        <nav>
          <div v-for="module in modules" :key="module.name" class="mb-2 my-1">
            <button @click="module.open = !module.open" class="flex items-center w-full justify-between p-2 rounded-md hover:bg-gray-200 dark:hover:bg-blue-700">
...
/>
  ...
  import { Menu, User, Settings, ChevronDown, ChevronUp, Moon, Sun, FileText,Briefcase,ShoppingCart , Calendar ,Mic ,MessageSquare  } from 'lucide-vue-next'
  ...
  const modules = ref([
    { name: 'Usuarios', icon: User, open: false, options: ['Lista Completa', 'Nuevos'] },
    { name: 'Roles', icon: FileText, open: false, options: ['Lista', 'Asignacion'] },
    { name: 'Permisos', icon: Briefcase, open: false, options: ['Lista', 'Asignacion'] },
    { name: 'Reportes', icon: ShoppingCart, open: false, options: [] },
    { name: 'Asociaciones', icon: Calendar, open: false, options: ['Admin', 'Productos'] },
    { name: 'Tiendas', icon: Mic, open: false, options: ['Latest', 'Trending'] },
    { name: 'Espacios Publicitarios', icon: MessageSquare, open: false, options: [] }
  ])
  </script>
